...
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-natty.sh $FF_TARGET
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-natty.sh $ARCH
    done
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        cd natty-$ARCH && git clean -xdf && cd -
    done
else
    echo "Usage:"
    echo "  compile-natty.sh armv7|arm64|i386|x86_64"
    echo "  compile-natty.sh armv7s (obselete)"
    echo "  compile-natty.sh lipo"
    echo "  compile-natty.sh all"
    echo "  compile-natty.sh clean"
    echo "  compile-natty.sh check"
    exit 1
fi
